...
```{r}
#Table(rawDF$label)
#this syntax shoudld be written in small letter like 
table(rawDF$label)
```
...
# the space between to lower should be deleted. So it will be like
...
#%>% tm_map(toSpace, "-") %>% tm_map(toSpace, "”")  %>% tm_map(toSpace, "“") %>% tm_map(toSpace, "’s") %>% tm_map(toSpace, " —")

# as these are not necessary, I deleted
```

```{r}
#cleanCorpus <- cleanCorpus %>% tmmap(stripWhitespace)

#tmmap should be written with underbar like tm_map
...
#inspect(cleanDTM)
#Documenttermmatrix, matrix's M should be in capital letter 
cleanDTM <- cleanCorpus %>% DocumentTermMatrix
inspect(cleanDTM)
```
```{r}
...
#This part, lapalace should be 1
